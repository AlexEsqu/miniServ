...
TEST_CASE("REQUEST")
{
	SUBCASE("Request constructor extracts correct values") {
	...
	SUBCASE("Request constructor use config to check method") {
	...
	SUBCASE("Incremental parsing of normal HTTP request") {
		ServerConf config;
		Status		stat;
	...
	SUBCASE("Incremental parsing of chunked HTTP request") {
	...
	SUBCASE("Parsing request with headers arriving one line at a time") {
		ServerConf config;
		Status		stat;
		Request req(config, stat, "");
	...
	SUBCASE("Parsing request with no body") {
		ServerConf config;
		Status		stat;
		Request req(config, stat, "");
	...
	SUBCASE("Parsing HTTP request with delimiter in body") {
		ServerConf config = generateDefaultConfig();
		Status		stat;
		Request req(config, stat, "");
